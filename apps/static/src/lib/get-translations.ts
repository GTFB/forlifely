 ...
 * Loads translations from pages/{pageName}.json file
 * Uses static imports for Cloudflare Pages compatibility
 */
export async function getPageTranslations(pageName: string, locale?: string): Promise<Translations> {
  const targetLocale = (locale || PROJECT_SETTINGS.defaultLanguage).toLowerCase()

  try {
    // Use explicit imports for Cloudflare Pages compatibility
    // Next.js will statically analyze these imports during build
    ...
    switch (pageName) {
      ...
      default:
        // Try dynamic import as fallback (may not work on Cloudflare Pages)
        try {
