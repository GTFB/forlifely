...
export class BaseMovesRepository extends BaseRepository<BaseMove> {
  ...
  private constructor(db: D1Database | SiteDb) {
    super(db, schema.baseMoves);
  }

  public static getInstance(db: D1Database | SiteDb): BaseMovesRepository {
    if (!BaseMovesRepository.instance) {
      BaseMovesRepository.instance = new BaseMovesRepository(db);
    }
  ...
  async recalculateReceivingMetrics(fullBaid: string): Promise<BaseMove | null> {
    ...
    inventoryList.forEach((item: RelationInventory) => {
      const quantity = toNumber(item.data_in.quantity ?? item.data_in.temp_quantity);
      if (!quantity) {
        return;
      }
  ...
  async softDeleteByFullBaid(fullBaid: string): Promise<boolean> {
    ...
    try {
      ...
      const relationsRepo = RelationsRepository.getInstance(this.db);
  ...
  async removeInventoryItem(fullBaid: string, itemUuid: string, isManager: boolean = false): Promise<BaseMove | null> {
    ...
    const relationRepository = RelationsRepository.getInstance(this.db);
  ...
  async addInventoryToLocation(opts: {
    ...
    data.items_count = inventory.reduce((sum, item) => {
      const qty = toNumber(item.data_in.quantity ?? item.data_in.temp_quantity);
      return sum + qty;
    }, 0);
  ...
  async recalculateSendingMetrics(fullBaid: string): Promise<BaseMove | null> {
    ...
    inventoryList.forEach((item) => {
      const quantity = Math.abs(toNumber(item.data_in.quantity ?? item.data_in.temp_quantity));
      if (!quantity) {
        return;
      }
      ...
      const sellingPriceFact = toNumber(item.data_in.selling_price);
      const purchasePriceFact = toNumber(item.data_in.purchase_price);
    ...
    inventoryList.forEach((item) => {
      ...
      if (sellingPricePerItem) {
        ...
        const currentPurchasePrice = toNumber(item.data_in.purchase_price);
        if (!currentPurchasePrice) {
          item.data_in.purchase_price = sellingPricePerItem;
        }
      }
      ...
      if (purchasePricePerItem) {
        ...
        const currentPurchasePriceNet = toNumber(item.data_in.purchase_price_net);
        if (!currentPurchasePriceNet) {
          item.data_in.purchase_price_net = purchasePricePerItem;
        }
      }
    });
    ...
    const walletsRepo = WalletsRepository.getInstance(this.db);
  ...
  async syncSendingToReceiving(sendingFullBaid: string): Promise<void> {
    ...
    for (const sendingItem of sendingInventory) {
      ...
      if (existingIndex >= 0) {
        ...
        receivingInventory[existingIndex].data_in.quantity = sendingItem.data_in.quantity;
        receivingInventory[existingIndex].data_in.temp_quantity = sendingItem.data_in.temp_quantity;
        ...
        const newItem = {
          ...
          data_in: {
            quantity: sendingItem.data_in.quantity,
            temp_quantity: sendingItem.data_in.temp_quantity,
          } as RelationInventoryDataIn,
          statusName: 'INCOME_INV',
        };

        // Copy prices
        if (sendingItem.data_in) {
          if (sendingItem.data_in.purchase_price !== undefined) {
            newItem.data_in.purchase_price = sendingItem.data_in.purchase_price;
          }
        }
  ...
  private async recalculateProductsForBaseMove(fullBaid?: string | null): Promise<void> {
    ...
    for (const paid of uniquePaid) {
      try {
        ...
        const productsRepo = ProductsRepository.getInstance(this.db);
  ...
  async sendPushToManager(title: string, body: string, context: { env: Env }, url: string): Promise<void> {
    const humanRepository = HumanRepository.getInstance(this.db);
