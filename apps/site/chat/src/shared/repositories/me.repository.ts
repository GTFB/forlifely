...
export class MeRepository {
  ...
  public static getInstance(db: D1Database | SiteDb): MeRepository {
  ...
  async findByIdWithRoles(id: number, options?: { includeHuman?: boolean, includeEmployee?: boolean, includeLocation?: boolean }): Promise<UserWithRoles | null> {
    ...
    if (employee && options?.includeLocation !== false) {
      try {
        const dataIn = employee.dataIn as Record<string, unknown> | null
        if (dataIn?.locationLaid) {
          location = await this.getLocation(dataIn.locationLaid as string)
        }
      } catch (err) {
