...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Используем правильное имя поля для обновления
    const dataInField = promocode.data_in !== undefined ? 'data_in' : 'dataIn'
    const updatedAtField = promocode.updated_at !== undefined ? 'updated_at' : 'updatedAt'
    
    // Экранируем JSON для SQL
    const dataInJson = JSON.stringify(updatedDataIn).replace(/'/g, "''")
    
    await env.DB.execute(
      sql.raw(`UPDATE "outreaches" 
          SET "${dataInField}" = '${dataInJson}'::jsonb,
              "${updatedAtField}" = NOW()
    ...
    if (promocode.title) {
      try {
        let titleRaw = promocode.title
        if (typeof titleRaw === 'string') {
          try {
            let parsed = JSON.parse(titleRaw)
            if (typeof parsed === 'string') {
              parsed = JSON.parse(parsed)
            }
