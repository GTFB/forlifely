    ...
    async (abortSignal?: AbortSignal, isMountedRef?: { current: boolean }) => {
      ...
      try {
        ...
        let extendedColumns: ColumnSchemaExtended[] = json.schema.columns.map((col) => {
          ...
          const dataTableFieldTitle = (translations as any)?.fields?.[collection]?.[col.name] as string | undefined
          ...
          if (!fieldTitle && collection === "roles" && col.name === "xaid") {
            ...
            console.warn(
              "[DataTable] Using hardcoded fallback for roles.xaid, translations available:",
              !!translations?.fields?.roles
            )
          }
          ...
          return {
            ...
            fieldType: shouldBeSelect
              ? "select"
              : forcedFieldType || options.type || fieldConfig?.type || inferredDbFieldType,
