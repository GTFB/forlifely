...
const Hero91 = ({
  ...
  const { title, description, button1, button2, subtitle, imageSrc } = content;
  const img = imageSrc ?? DEFAULT_IMAGE;
    ...
    >
      ...
      <div className="absolute right-0 top-1/2 hidden h-[720px] w-[45%] min-w-[320px] -translate-y-1/2 overflow-hidden rounded-l-full bg-black lg:block">
        <img
          src={img}
          alt=""
          className="h-full w-full rounded-tl-xl object-cover object-top"
        />
      </div>
    </section>
  );
